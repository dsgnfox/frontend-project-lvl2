...
test('findDifferencesYAML', () => {
  formatters.forEach((formatter) => {
    const diff = gendiff(files.beforeFileJSON, files.afterFileYML, formatter);
    expect(diff).toEqual(result[formatter]);
  });
});
